...
import type { ChildrenData } from '../../utils/statistics';

interface ChildrenBarChartProps {
  data: ChildrenData[];
  totalCount: number;
}

export function ChildrenBarChart({ data, totalCount }: ChildrenBarChartProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      <div className="mt-4 text-xs flex-shrink-0" style={{ color: 'var(--text-tertiary)' }}>
        총 {totalCount.toLocaleString()}명 중 기혼인 {data.reduce((sum, item) => sum + item.count, 0)}명 기준
      </div>
    </div>
  );
}
